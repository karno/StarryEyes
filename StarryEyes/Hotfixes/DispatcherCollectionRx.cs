...
namespace StarryEyes.Hotfixes
{
    ...
    public class DispatcherCollectionRx<T> : IList<T>, ICollection, INotifyCollectionChanged, INotifyPropertyChanged, IDisposable
    {
        private readonly IList<T> _souceAsIList;
        ...
        public DispatcherCollectionRx(Dispatcher dispatcher) : this(new ObservableCollection<T>(), dispatcher) { }
        ...
        protected void ThrowExceptionIfDisposed()
        {
            if (_disposed)
            {
                throw new ObjectDisposedException("DispatcherCollection");
            }
        }

        ~DispatcherCollectionRx()
        {
            Dispose(false);
        }
    }
